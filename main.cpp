...
bool dfs(int r, int c, vector<vector<int>>& maze, vector<vector<bool>>& visited, vector<vector<int>>& parent_r, vector<vector<int>>& parent_c, int exit_r, int exit_c){
    ...
    if (r == exit_r && c == exit_c) {    //check for the exit
        return true;
    }

    for (int i = 0; i < 4; i++) {   //go in all four directions
        int newRow = r + dr[i];     //uses both directional arrays
        int newCol = c + dc[i];

        //handle edge cases
        if (newRow >= 0 && newRow < height && newCol >= 0 && newCol < width && maze[newRow][newCol] == 0 && !visited[newRow][newCol])
        {
...
int main() {
    ...
    cout << "Enter maze dimensions height N and width M: ";
    cin >> N >> M;
